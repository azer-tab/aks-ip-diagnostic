...
def run_pod_analysis(cluster_name: str, resource_group: str, subscription_id: str):
    ...
    status_emoji = {"LOW": "🟢", "OPTIMAL": "🟢", "HIGH": "🟡", "CRITICAL": "🔴"}
    print("\n🎯 Pod Density:")
    print(
        f"  Status: {status_emoji.get(density['density_status'], '❓')} {density['density_status']}"
    )
    ...
    for ns in analysis["namespace_analysis"]["namespaces"][:5]:
        print(
            f"  {ns['namespace']:<30} {ns['pod_count']:>4} pods | "
    ...
    for node in analysis["node_analysis"][:5]:
        util_emoji = (
            "🔴"
            if node["utilization_percentage"] > 90
            else "🟡"
            if node["utilization_percentage"] > 70
            else "🟢"
        )
        print(
            f"  {util_emoji} {node['node_name']:<40} "
...
def generate_pod_analysis_report(analysis: dict, output_format: str = "json-pretty"):
        ...
        issues_list.append(
            {
                ...
                "details": {"description": issue["description"], "impact": issue["impact"]},
            }
        )
    ...
    builder.set_summary(
        ...
        if analysis["ip_waste_analysis"]["waste_level"] == "CRITICAL"
        else "WARNING",
        risk_level=analysis["ip_waste_analysis"]["waste_level"],
    )
...
def main():
    ...
    try:
        # Run main analysis
        analysis = run_pod_analysis(args.cluster_name, args.resource_group, args.subscription_id)
