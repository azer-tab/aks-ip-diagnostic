...
class PodIPAnalyzer:
    ...
    def get_all_pods(self) -> list[dict]:
        """Return all pods from the Kubernetes client as plain dictionaries.

        The analyzer deliberately keeps collection separate from calculations.
        Tests can call the calculation methods directly with fixture dictionaries,
        while production scans use this read-only client call.
        """
        if not self.k8s_client or not hasattr(self.k8s_client, "list_pods_all_namespaces"):
            return []
        return self.k8s_client.list_pods_all_namespaces()

    def get_all_nodes(self) -> list[dict]:
        """Return all cluster nodes from the Kubernetes client as dictionaries."""
        if not self.k8s_client or not hasattr(self.k8s_client, "list_nodes"):
            return []
        return self.k8s_client.list_nodes()

    def analyze_pod_distribution(self, pods: list[dict], nodes: list[dict]) -> dict:
        """Calculate how evenly pods are distributed across nodes.

        Unbalanced distribution can indicate scheduler constraints, node
        affinity rules, topology spread issues, or resource hotspots. The
        method only depends on pod/node dictionaries, which makes it safe to
        unit-test without a live Kubernetes cluster.
        """
        node_names = [
        ...
        node_pod_count = {node_name: 0 for node_name in node_names}

        # Count scheduled pods per node. Pods without spec.nodeName are still
        # pending scheduling and should not increase any node's load.
        for pod in pods:
            node_name = pod.get("spec", {}).get("nodeName")
            if node_name:
                node_pod_count.setdefault(node_name, 0)
        ...
        variance = sum((count - avg_pods) ** 2 for count in pod_counts) / len(pod_counts)
        std_dev = variance**0.5

        # A cluster is considered balanced when the spread is less than 20% of
        # the mean. This is intentionally a heuristic rather than a hard AKS
        # rule because workload scheduling constraints vary widely.
        is_balanced = std_dev < (avg_pods * 0.2) if avg_pods > 0 else True
        imbalance_pct = (std_dev / avg_pods * 100) if avg_pods > 0 else 0
        nodes_running_pods = sum(1 for count in pod_counts if count > 0)
    ...
    def analyze_ip_allocation(self, pods: list[dict]) -> dict:
        ...
        return {
            ...
            "unique_ips": len(
                set(ip_addresses)
            ),  # Count of distinct IPs (should equal pods_with_ip)
    ...
    def analyze_by_namespace(self, pods: list[dict]) -> dict:
        ...
        for ns, stats in namespace_stats.items():
            ...
            ip_alloc_rate = (
                (stats["pods_with_ip"] / stats["pod_count"] * 100) if stats["pod_count"] > 0 else 0
            )

            # Calculate health percentage (how many pods are running successfully)
            health_pct = (
                (stats["running_pods"] / stats["pod_count"] * 100) if stats["pod_count"] > 0 else 0
            )
        ...
        return {
            ...
            "system_namespaces": [ns for ns in namespace_stats if ns.startswith("kube-")],
        }
    ...
    def analyze_by_node(self, pods: list[dict], nodes: list[dict]) -> list[dict]:
        ...
        for node in nodes:
            ...
            node_info[node_name] = {
                ...
                "allocatable_pods": int(
                    node.get("status", {}).get("allocatable", {}).get("pods", 0)
                ),
        ...
        for node_name, stats in node_stats.items():
            # Get node capacity info we collected earlier. Prefer allocatable pods
            # because it is the kubelet-enforced limit; fall back to capacity if
            # allocatable is missing from older or partial node payloads.
            info = node_info.get(node_name, {})
            allocatable = info.get("allocatable_pods") or info.get("capacity_pods") or 0
    ...
    def calculate_pod_density(self, pods: list[dict], nodes: list[dict]) -> dict:
        ...
        """
        # Count only Running pods (exclude Pending, Failed, Succeeded). This
        # keeps density focused on active workloads rather than completed Jobs.
        total_pods = sum(1 for pod in pods if pod.get("status", {}).get("phase") == "Running")
        total_nodes = len(nodes)

        if total_nodes == 0:
            return {"pods_per_node": 0, "density_status": "N/A", "recommendation": "No nodes found"}
        ...
        if pods_per_node < 10:
            status = "LOW"
            recommendation = (
                "Cluster is under-utilized. Consider reducing node count or scaling workloads."
            )
            ...
            status = "CRITICAL"
            recommendation = (
                "Pod density is critically high. Add nodes immediately to prevent issues."
            )
    ...
    def analyze_ip_waste(self, pods: list[dict], nodes: list[dict]) -> dict:
        ...
        cost_impact = (
            "High" if waste_percentage > 50 else "Medium" if waste_percentage > 30 else "Low"
        )
                ...
                host_network_pods.append(
                    {
                        ...
                        "uses_host_ip": pod.get("status", {}).get("podIP")
                        == pod.get("status", {}).get("hostIP"),
                    }
                )
    ...
    def _determine_multi_ip_reason(self, pod: dict) -> str:
        ...
        if len(pod_ips) >= 2:
            try:
                ...
                if (ip1.version == 4 and ip2.version == 6) or (
                    ip1.version == 6 and ip2.version == 4
                ):
        ...
        if any("network" in key.lower() for key in annotations):
    ...
    def detect_issues(self, analysis: dict) -> list[dict]:
            ...
            issues.append(
                {
                    ...
                    "impact": waste.get(
                        "impact",
                        "Inefficient IP usage may lead to subnet exhaustion and increased costs",
                    ),
                    "affected_resources": ["cluster-wide"],
        ...
        if density.get("density_status") in ["HIGH", "CRITICAL"]:
            issues.append(
                {
                    "severity": "WARNING"
                    if density.get("density_status") == "HIGH"
                    else "CRITICAL",
    ...
    def generate_recommendations(self, analysis: dict) -> list[dict]:
        ...
        distribution = analysis.get("pod_distribution", {})
        if (
            not distribution.get("balanced", True)
            and distribution.get("imbalance_percentage", 0) > 30
        ):
            imbalance = distribution.get("imbalance_percentage", 0)
            recommendations.append(
                {
