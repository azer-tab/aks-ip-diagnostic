...
def analyze_provisioning_failure(pool) -> dict | None:
    """
...
def calculate_ip_deficit(
    required_ips: int, available_ips: int, buffer_percent: float = 20.0
) -> dict:
    """
    ...
    current_coverage = (
        (available_ips / recommended_capacity * 100) if recommended_capacity > 0 else 0
    )
...
def detect_upgrade_blocking_exhaustion(node_pools: list, network_client, logger) -> list[dict]:
    """
...
def check_ip_exhaustion(
    aks_client, cluster, node_pools: list, network_client, logger
) -> list[dict]:
    """
