...
class DiagnosticReportBuilder:
    ...
    def add_diagnostic_result(self, diagnostic_type: str, status: str, risk_level: str, 
                            issues: List[Dict], details: Dict = None):
    ...
    def set_summary(self, overall_status: str, risk_level: str, 
                   health_score: Dict = None, efficiency_metrics: Dict = None,
                   cost_impact: Dict = None, capacity_outlook: Dict = None):
...
def create_issue(severity: str, code: str, message: str, 
                affected_resource: str = None, details: Dict = None, 
                remediation: str = None) -> Dict:
...
def create_recommendation(priority: str, category: str, title: str, 
                         description: str, affected_resources: List[str] = None,
                         impact: str = None, recommendation: str = None,
                         steps: List[str] = None, downtime: str = None,
                         automation: bool = False, docs: List[str] = None) -> Dict:
...
def _format_cost_analysis_text(cost_data: Dict) -> List[str]:
    ...
    lines.append("\nCOST ANALYSIS:")
    lines.append(f"  Status: {cost_data.get('status')} (Financial Impact: {cost_data.get('risk_level')})")
    ...
    if waste_costs:
        ...
        lines.append("\n  💰 IP Waste Costs:")
        lines.append(f"    Unused IPs:   {waste_costs.get('unused_ip_count', 0)} ({waste_pct:.1f}% waste)")
        ...
        if waste_pct > 20:
            lines.append(f"    ⚠️  High waste detected - over 20% of IP costs are wasted!")
...
def format_text_report(report_data: Dict) -> str:
    ...
    if node_pools:
        ...
        for pool in node_pools:
            ...
            lines.append(f"  Required IPs: {required_ips} [{max_pods} pods + 1 node] × {node_count} nodes")
...
def format_markdown_report(report_data: Dict) -> str:
    ...
    lines.append("## Summary")
    lines.append("")
    status_emoji = "🟢" if summary.get('overall_status') == 'HEALTHY' else "🔴" if summary.get('overall_status') == 'CRITICAL' else "🟡"
    ...
    for diag_name, diag_data in diagnostics.items():
        ...
        if issues:
            for issue in issues:
                severity_emoji = "🔴" if issue.get('severity') in ['CRITICAL', 'ERROR'] else "🟡" if issue.get('severity') == 'WARNING' else "ℹ️"
