...
from .json_schema import DIAGNOSTIC_REPORT_SCHEMA, NODE_POOL_ANALYSIS_SCHEMA, SUBNET_ANALYSIS_SCHEMA
...
class ReportEnricher:
    ...
    def enrich_subnet_data(subnet_data: dict) -> dict:
        ...
        if (
            "usage_percentage" not in enriched
            and "available_ips" in enriched
            and "address_space_size" in enriched
        ):
    ...
    def enrich_diagnostic_report(report_data: dict) -> dict:
        ...
        if "node_pools" in enriched:
            enriched["node_pools"] = [
                ReportEnricher.enrich_node_pool_data(pool) for pool in enriched["node_pools"]
            ]

        # Enrich subnets
        if "subnets" in enriched:
            enriched["subnets"] = [
                ReportEnricher.enrich_subnet_data(subnet) for subnet in enriched["subnets"]
            ]
